...
def mazeTraversal():
    ...
    while len(visited) != len(world.rooms):
        ...
        print(unvisited)
        # if unvisited is not empty, choose a random direction and move there
        # print(visited[roomId])
        if len(unvisited) > 0:
            # grab a random unvistied room
            nextDirectionIndex = random.randint(1, len(unvisited))
            nextDirection = unvisited[nextDirectionIndex - 1]

            # move to a new room
            print('before', player.currentRoom.id, nextDirection)
            player.travel(nextDirection)
            print('after', player.currentRoom.id, nextDirection)
            # add direction to treaversal path
            traversalPath.append(nextDirection)
            # set the previous room direction and new room direction accordingly
            nextRoomId = player.currentRoom.id
            visited[roomId][nextDirection] = nextRoomId
            # if new room not visited add it to visited
            if nextRoomId not in visited:
                nextExits = player.currentRoom.getExits()
                visited[nextRoomId] = {
                    direction: '?' for direction in nextExits}
            # add direction to new room
